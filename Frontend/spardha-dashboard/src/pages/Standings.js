  ...
  return (
    <div className="p-4">
      <div className="standings_header d-flex flex-row align-items-center">
        ...
        <select onChange={sportHandler} className="standings_dropdown" name='' id=''>
          <option value={-1} className="standings_dropdown_text">Overall</option>
          {sports && sports.map((sport, i) => (
            <option value={sport.id} className="standings_dropdown_text">{sport.name}</option>
          ))}
        </select>
